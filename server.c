 ...
 *   server
 *
 * waits for client to connect, receive two integers, and sends their
...
int main(int argc, char *argv[])
{
	...
	mr = ibv_reg_mr(pd, buf, 2 * sizeof(uint32_t), IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ);
	if (!mr)
		return 1;
